...
fn test_dvb_iso8859_extended() {
    // ISO-8859-5 via extended selection (0x10 0x00 0x05)
    let data = [0x10, 0x00, 0x05, 0xbf, 0xe0, 0xd8, 0xd2, 0xd5, 0xe2, 0x21];
    let text = decode(&data);
    assert_eq!(text, "Привет!");
}
...
fn test_dvb_utf8() {
    // UTF-8 prefix
    let data = [
        0x15, 0xd0, 0x9f, 0xd1, 0x80, 0xd0, 0xb8, 0xd0, 0xb2, 0xd0, 0xb5, 0xd1, 0x82, 0x21,
    ];
    let text = decode(&data);
    assert_eq!(text, "Привет!");
}
